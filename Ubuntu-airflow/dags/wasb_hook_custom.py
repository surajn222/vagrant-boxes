...
class WasbHookCustom(BaseHook):
    ...
    def __init__(
        self,
        variable_account_name: str = None,
        variable_shared_key: str = None,
        public_read: bool = False,
    ) -> None:
        super().__init__()
        self.variable_account_name = variable_account_name
        self.variable_shared_key = variable_shared_key
    ...
    def get_conn(self) -> BlobServiceClient:
        ...
        account_name = Variable.get(self.variable_account_name)
        account_url = f"https://" + account_name + ".blob.core.windows.net/"
        shared_key = Variable.get(self.variable_shared_key)
    ...
    def walk_container(self, container_name, prefix):
        #container_client = client.get_container_client(container.name)
        container_client = self._get_container_client(container_name)
        print('C: {}'.format(container_name))
        depth = 1
        separator = '   '
        from azure.storage.blob import BlobPrefix
        list_blobs = []
        def walk_blob_hierarchy(prefix=prefix):
            nonlocal depth
            for item in container_client.walk_blobs(name_starts_with=prefix):

                short_name = item.name[len(prefix):]
                if isinstance(item, BlobPrefix):
                    #print('Immediate Folder name: ' + separator * depth + short_name)
                    depth += 1
                    walk_blob_hierarchy(prefix=item.name)
                    depth -= 1
                else:
                    print("Full file name to be used: " + str(item.name))
                    list_blobs.append(str(item.name))
                    message = 'File name: ' + separator * depth + short_name
    ...
    def get_blob_bytes(self, container_name, file_name):
        container_client = self._get_container_client(container_name)
